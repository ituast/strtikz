...
\ifthenelse{\showdefl=1}{
	...
	{
		\draw [line width = \collinet/2]
		(\x{\icol},\y{1}) .. controls +(0.0cm, \defbase) and +(-\defparx, -\defpary) ..
		(\x{\icol}, \y{2});
	}
	\foreach \ibeam in {1,...,{\baynumber}}
	{
		\draw [line width = \beamlinet/2]
		(\x{\ibeam}-\fixbeamx, \y{2}+\fixbeamy) .. controls +(\defparbeamy,-\defparbeamx) and +(-\defparbeamy, +\defparbeamx) ..
		(\usevar\x{\ibeam+1}+\fixbeamx , \y{2}-\fixbeamy);
	}
	...
	\foreach \istory in {2,...,{\storyminone}}
	{
		...
		{
			\draw [line width = \collinet/2]
			(\x{\icol},\y{\istory}) .. controls +(\defparx, \defpary) and +(-\defparx, -\defpary) ..
			(\usevar\x{\icol}, \usevar\y{\istory+1});
		}
		\foreach \ibeam in {1,...,{\baynumber}}
		{
			\draw [line width = \beamlinet/2]
			(\x{\ibeam}-\fixbeamx, \usevar\y{\istory+1}+\fixbeamy)
			.. controls +(\defparbeamy,-\defparbeamx) and +(-\defparbeamy, +\defparbeamx) ..
			(\usevar\x{\ibeam+1}+\fixbeamx, \usevar\y{\istory+1}-\fixbeamy);
		}
	}
