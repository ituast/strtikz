...
\ifthenelse{\storynumber>1}{
	\foreach \iii in {2,...,{\nlevmo}}
	{\draw [\beamcolor, line width = \beamlinet] (\x{1},\y{\iii}) -- 
		(\x{\columnnumber},\y{\iii});}}{}

%% Draw the beam the the top floor
\draw [line width = \beamlinet, \beamcolor]
(\x{1}-\collinet/2, \y{\levelnumber})
-- (\x{\columnnumber}+\collinet/2, \y{\levelnumber});

%% Draw the first column
\draw [line width = \collinet, \columncolor]
(\x{1},\y{1}-\baselinet/2) -- (\x{1}, \y{\levelnumber}+\beamlinet/2);

%% Columns other than first and last columns
\foreach \j in {2,...,{\ncolmo}}
{\draw [line width = \collinet, \columncolor]
	(\x{\j},\y{1}-\baselinet/2) -- (\x{\j},\y{\levelnumber}+\beamlinet/2);}

%% Draw the last column
\draw [line width = \collinet, \columncolor]
(\x{\columnnumber}, \y{1}-\baselinet/2)
-- (\x{\columnnumber}, \y{\levelnumber}+\beamlinet/2);
...
\ifthenelse{\subfloors>1}{
	...
	{\draw [blue, line width = \beamlinet, \beamcolor] (\x{1},\y{\iii}) -- 
		(\x{\columnnumber},\y{\iii});}
...
%% If the superstory number is different then zero, firs draw the first story
...
\ifthenelse{\showsupports=5}{
	...
	\ifthenelse{\showmarkss=1}{
		...
		line width=\marksslinet, color=black, \marksslinetype]
