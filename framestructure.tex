...
{\deflstart=\subfloors+1;} else {\deflstart=1;};
...
\ifthenelse{\showsupports=5}{
	...
	%%% Transmitting Layers
	\draw[line width=\translayerlinet, red, dashed]
	...
	%%%Soil-Structure Interface
	\draw[line width=\ssinterlinet, green, dashed]
	...
	\node[align=center, scale=1] at (\engbedrockmidx, \engbedrockmidy) {Bedrock};
...
\ifthenelse{\storynumber>1}{
	\foreach \iii in {2,...,{\nlevmo}}
	{\draw [blue, line width = \beamlinet/2] (\x{1},\y{\iii}) -- 
	...
	\ifthenelse{\showsupports=5}{
		...
		\ifthenelse{\subfloors>0}{
			...
			{\foreach \j in {2,...,\useeval{\columnnumber-1}}{
				\shade[ball color=red] (\x{\j},\y{\iii}) circle (\massrad);
			}}
		}{}
	}
	{
...
\draw [line width = \collinet/2]
(\x{1},\y{1}) -- (\x{1}, \y{\deflstart});

\draw [line width = \beamlinet/2]
(\x{1}-\collinet/2/2, \y{\deflstart}) -- (\x{\columnnumber}+\collinet/2/2, \y{\deflstart});

\draw [line width = \collinet/2]
(\x{\columnnumber}, \y{\deflstart}) -- (\x{\columnnumber}, \y{1});


\foreach \j in {2,...,{\ncolmo}}
{\draw [line width = \collinet/2] (\x{\j},\y{1}) -- (\x{\j},\y{\deflstart});}
...
\foreach \ibeam in {1,...,{\baynumber}}
{
	\draw [red, line width = \beamlinet/2]
...
\foreach \icol in {1,...,{\columnnumber}}
{
	...
	(\x{\icol}+\deflect{\deflstartplusone}, \y{\deflstartplusone});
}}{}
...
\ifthenelse{\superstorynumber>1}{
	\ifthenelse{\superstorynumber>2}{
		\foreach \istory in {\deflstartplusone,...,{\storyminone}}
		{
			\foreach \ibeam in {1,...,{\baynumber}}
			{
				\draw [blue, line width = \beamlinet/2]
