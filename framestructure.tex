...
real \drift, \deflect, \defparx, \defpary, \defparbeamx, \defparbeamy, \defbase, \tempdim, \fixbeamx, \fixbeamy ;
...
\fixbeamx = \framelinet/2*\defpary/\tempdim;
\fixbeamy = \framelinet/2*\defparx/\tempdim;
...
\ifthenelse{\latloadtype=5}
{
	...
	{
		\draw [arrows={-[length=5pt, width=4pt]>}, line width=1pt]
		(\x{1}+\deflect{\istory}-\toparrlen,\y{\istory}) -- (\x{1}+\deflect{\istory}-0.1cm, \y{\istory});
	}
}{;}
}{;}
...
\ifthenelse{\showdefl=1}{
	...
	\foreach \istory in {2,...,{\storynumber}}
	{
		\foreach \icol in {1,...,{\columnnumber}}
		{
			\draw [line width = \framelinet]
