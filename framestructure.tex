...
real \drift, \deflect, \defparx, \defpary;
...
\foreach \j in {2,...,{\ncolmo}}
{\draw [line width = \framelinet, color=red] (\x{\j},\y{1}) -- (\x{\j},\y{\levelnumber});}
}{}
...
\ifthenelse{\showdefl=3}{
	\foreach \istory in {1,...,{\storynumber}}
	{
		...
		{
			\draw [line width = \framelinet]
			(\x{\ibeam}-\framelinet/2,\usevar\y{\istory+1}) -- (\usevar\x{\ibeam+1}+\framelinet/2, \usevar\y{\istory+1});
		}
...
\ifthenelse{\showdefl=4}{
	\foreach \istory in {1,...,{\storynumber}}
	{
		\foreach \icol in {1,...,{\columnnumber}}
		{
			\draw [line width = \framelinet]
			(\x{\icol}+\deflect{\istory},\y{\istory}) -- (\usevar\x{\icol}+\usevar\deflect{\istory+1}, \usevar\y{\istory+1});
		}
		\foreach \ibeam in {1,...,{\baynumber}}
		{
			\draw [line width = \framelinet]
			(\x{\ibeam}-\framelinet/2+\usevar\deflect{\istory+1}, \usevar\y{\istory+1}) --
			(\usevar\x{\ibeam+1}+\framelinet/2+\usevar\deflect{\istory+1}, \usevar\y{\istory+1});
		}
...
\ifthenelse{\showdefl=1}{
	\foreach \istory in {1,...,{\storynumber}}
	{
		...
		{
			\draw [line width = \framelinet]
			(\x{\icol}+\deflect{\istory},\y{\istory}) .. controls +(0.0cm, 0.15cm) and +(-\defparx, -\defpary) ..
		...
		{
			\draw [line width = \framelinet]
			(\x{\ibeam}+\usevar\deflect{\istory+1}, \usevar\y{\istory+1}) .. controls +(\defpary,-\defparx) and +(-\defpary, +\defparx) ..
			(\usevar\x{\ibeam+1}+\usevar\deflect{\istory+1}, \usevar\y{\istory+1});
		}
