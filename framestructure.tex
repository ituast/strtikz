\pgfkeys{
 /framestructure/.is family, /framestructure,
  ...
  beam line thickness/.store in = \beamframelinet,
  column line thickness/.store in = \colframelinet,
...
real \beamframelinet, \colframelinet;
...
\linet = \linet;
\beamframelinet = \beamframelinet;
\colframelinet = \colframelinet;
\baselinet = \baselinet;
\isolinet = \isolinet;
\massrad = \massrad;
...
\fixbeamx = \colframelinet/2*\defpary/\tempdim;
\fixbeamy = \colframelinet/2*\defparx/\tempdim;
...
\draw [line width = \colframelinet]
(\x{1},\y{1}) -- (\x{1}, \y{\levelnumber});
\draw [line width = \beamframelinet]
(\x{1}-\colframelinet/2, \y{\levelnumber}) -- (\x{\columnnumber}+\colframelinet/2, \y{\levelnumber});
\draw [line width = \colframelinet]
(\x{\columnnumber}, \y{\levelnumber}) -- (\x{\columnnumber}, \y{1});
 

\foreach \iii in {2,...,{\nlevmo}}
{\draw [line width = \beamframelinet] (\x{1},\y{\iii}) -- 
(\x{\columnnumber},\y{\iii});}

\foreach \j in {2,...,{\ncolmo}}
{\draw [line width = \colframelinet] (\x{\j},\y{1}) -- (\x{\j},\y{\levelnumber});}
}{}
...
\ifthenelse{\showdefl=3}{
	\foreach \istory in {1,...,{\storynumber}}
	{
		...
		{
			\draw [line width = \beamframelinet]
			(\x{\ibeam}-\colframelinet/2,\usevar\y{\istory+1}) -- (\usevar\x{\ibeam+1}+\colframelinet/2, \usevar\y{\istory+1});
		}
