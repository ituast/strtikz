...
\node at (-1,-2cm) []{\leny};
\node at (-1,-3cm) []{\alp};
\node at (-1,-4cm) []{\linelength};
\node at (-1,-5cm) []{\curvlen};
\node at (-1,-6cm) []{\curvang{1}};
\node at (-1,-7cm) []{\curvang{2}};

\draw (\startx,\starty) -- (\midx{1}, \midy{1})
.. controls ++(\curvang{1}:2*\curvlen) and ++(\curvang{2}:2*\curvlen) ..
(\midx{2}, \midy{2})
.. controls ++(\curvang{3}:2*\curvlen) and ++(\curvang{4}:2*\curvlen) ..
