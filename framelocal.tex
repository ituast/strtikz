...
if equal(\placeyzdofsonly,1) then {\localxstartoffset=\localxlength/5; \idofnodaloffsety=0;} else {};
if equal(\showdeep,1) then {
\jointinnersep=\jointinnersep+\beamdepth/2 + \linethickness;
...
\path [] (0,0)
node[inner sep=\jointinnersep, outer sep=0, anchor=\rotationangle+\startjointangle]{\jointtextfontsize \jointitext} -- (\lengthofelement,0)
node[inner sep=\jointinnersep, outer sep=0, anchor=\rotationangle+\endjointangle]{\jointtextfontsize \jointjtext}
...
%Place the local axes
\ifthenelse{\placelocalaxes=1}{
\draw [arrowcoord] (\localxstart,\localxshift) --
node[at end, inner sep=1pt, outer sep=0, anchor=\rotationangle+180]{\localaxesfontsize \localxtext}(\localxlength+\localxstart,\localxshift);
\draw [arrowcoord] (\localyshift,\localystart) --
node[at end, inner sep=1pt, outer sep=0, anchor=\rotationangle-90]{\localaxesfontsize \localytext}(\localyshift, \localystart+\localylength);
}{}

%Place the DOFs on the ith joint
\ifthenelse{\placeidofs=1}{

%Place only y and z dofs only
\ifthenelse{\placeyzdofsonly=1}
{\dofsyz[startx = \startoffsetx, starty = \startoffsety, length x = \lengthdofs,length y = \lengthdofs,
...
rotation=0, font rotation=\rotationangle,
dof arrow size ratio=\dofarrtipsizrat, dof arrow tip length=\dofarrtiplen, dof arrow tip width=\dofarrtipwid]}
%Place x y and z dofs
...
%Place the DOFs on the jth joint
\ifthenelse{\placejdofs=1}{

%Place only y and z dofs only
\ifthenelse{\placeyzdofsonly=1}
{\dofsyz[startx = \lengthofelement+\endoffsetx, starty = \endoffsety, length x = \lengthdofs,length y = \lengthdofs,
...
\draw [arrowcoord] (0,\globalYstart) -- node[at end, inner sep=1pt, outer sep = 0, above]{\globalaxesfontsize \globalytext}(0,\globalYend);
\end{scope}
}{}
